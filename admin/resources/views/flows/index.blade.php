...
<style>
    ...
    .tva-flow-row__name { font-size:15px; font-weight:600; color:var(--tva-text); }
...
<div class="content">
    ...
    <div class="tva-flow-card">
        <div class="flex items-center mb-4">
            ...
            {{-- Creating a flow asks for a name and nothing else. Designing it
                 — by hand or by describing it to the AI — happens in the
                 editor, where you can see what you're getting. --}}
...
<div id="flow-create" class="tva-modal" hidden>
    ...
    <form method="POST" action="{{ route('flows.store', ['client' => $client->slug]) }}" class="tva-modal__panel" style="max-width:520px;">
        ...
        <div class="tva-modal__body">
            ...
            {{-- Name only. Language lives in the editor's Flow settings, where
                 it sits next to the nodes it affects — asking for it here made
                 creating a flow feel like a form to fill in rather than one
                 click on the way to the canvas. --}}
            <div class="text-slate-500 text-xs">
                You'll design the flow in the editor — drag the steps yourself, or
...
<script>
    if (window.lucide?.createIcons) {
        try { window.lucide.createIcons({ icons: (window.lucide.icons || {}), nameAttr: 'data-lucide' }); } catch (_) {}
    }
